...
We are going to illustrate Bayesian GLMs with the example of Logistic regression models, which have a wide variety of applications in statistics, from text classification to medicine and genetic modelling.
...
Nevertheless, we can use a Gaussian prior \eqref{eq:NIGprior} or an (improper) flat prior  \eqref{eq:flat-prior} for $\btheta$, but both require approximate inference (see Section \ref{sec:logit-inf}).\\
...
\textbf{Regularization} can also be achieved with the same prior distributions as introduced for Bayesian linear regression in Section \ref{sec:lm-regularization} \Citep[see e.g.][]{van_erp_shrinkage_2019,fahrmeir_bayesian_2010,ohara_review_2009}.
...
The Metropolis–Hastings algorithm for $K$ samples\footnotemark proceeds as follows:
...
In contrast to MCMC methods, Laplace Approximation (LA) approximates the full posterior distribution by assuming a Gaussian distribution:
\begin{equation}
...
Under Laplace Approximation, we may
\begin{itemize}
    \item draw samples $\btheta_s \sim \Ncal(\hbtheta_{MAP}, H^{-1}(\hbtheta_{MAP}))$ with $s = 1, \dots, S$ and compute \eqref{eq:ppd-sample}.
