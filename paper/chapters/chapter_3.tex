...
Priors for the parameter $\btheta$ can be set in the same way as for the Bayesian linear model. However, in practice, the prior choice also depends on the link function \citep{west_dynamic_1985} (TODO: ELABORATE).

\subsection{Bayesian Logistic Model} \label{sec:logit-logit}

We are going to illustrate Bayesian GLMs with the example of logistic regression models, which have a wide variety of applications in statistics, from text classification to medicine and genetic modeling. (TODO: SOURCE)
...
for observations $\bx_i = (1, x_{i1}, \dots, x_{ip})$ and where $\sigma(y) = \frac{\exp(y)}{1 + \exp(y)}$ is the logistic (sigmoid) function.  Other choices like the probit link, which is common in econometrics (TODO: SOURCE) can also be used.
...
Unlike for the linear model, Bayesian inference with closed form posteriors is not possible in most cases. (TODO: SOURCE)
...
\citep{scott_data_2011} argues that using heavier-tailed proposals (e.g.\@ Student–$t$) can improve mixing (TODO: ELABORATE) by allowing larger moves.\\
\footnotetext{
